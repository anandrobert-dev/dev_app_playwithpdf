# gui/splitter_gui_final.py

import os
import datetime
from PyQt5.QtWidgets import (
    QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QFileDialog, QTableWidget,
    QSpinBox, QLineEdit, QDateEdit, QHeaderView, QMessageBox, QAbstractItemView, QFrame
)
...
class PDFSplitterGUI(QWidget):
    ...
    def add_row_widgets(self, row_pos, start_val=None):
        ...
        remove_btn.clicked.connect(lambda: self.table.removeRow(row_pos))
    ...
    def load_pdf(self, path):
        ...
        self.info_label.setText(
            f"📄 {os.path.basename(path)} | Pages: {self.total_pages} | Size: {file_size:.1f} MB"
        )
    ...
    def split_pdf(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if not (start_widget and end_widget and name_widget):
                continue
            ...
            if output_name in used_names:
                self.show_error(f"Duplicate file name: {output_name}")
                return

            if start > end:
                self.show_error(f"Start page must be <= end page (Row {row + 1})")
                return

            ranges.append((start, end, output_name))
        ...
        try:
            ...
            self.show_error(f"Permission denied to create output folder in:\n{os.path.dirname(self.pdf_path)}")
            return

        try:
            from pdf_utils.splitter import split_pdf_by_ranges
        except ImportError:
            self.show_error("Split function not found.")
            return
